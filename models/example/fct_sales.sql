...
SELECT 
  ...
  `Campaign Name`,
  `Source`,
  STRING_AGG(Sales_Agent_Name, ', ') AS `Sales Agents`, -- Aggregating agents into a list
  `Total Revenue`,
  `Rebill Revenue`,
  `Number Of Rebills`,
  `Discount Amount`,
...
FROM 
  base_data
GROUP BY 
  `Reference ID`, `Product Name`, `Country`, `Campaign Name`, `Source`, `Total Revenue`, `Rebill Revenue`, 
